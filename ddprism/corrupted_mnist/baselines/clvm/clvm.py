...
class clvmVAE(clvmLinear):
    ...
    def sample_latents(
        ...
        mu_tx, log_sigma_tx = self.vae_t.encode(x, a_mat)

        mu_zx, log_sigma_zx = self.vae_z.encode(x, a_mat)
        mu_zy, log_sigma_zy = self.vae_z.encode(y, a_mat)
    ...
    def expect_data(
        ...
        # Compute expected values of the target and background datasets.
        x = self.vae_z.decode(zx, a_mat) + self.vae_t.decode(tx, a_mat)
        y = self.vae_z.decode(zy, a_mat)
    ...
    def denoise_samples(
        ...
        x_denoised = self.vae_t.decode(tx, a_mat)
        return x_denoised
    ...
    def __call__(
        ...
        # Generate new samples of the data.
        # Sample latent variables.
        tx, zx, zy, latent_params = self.sample_latents(rng, x, y, a_mat)
