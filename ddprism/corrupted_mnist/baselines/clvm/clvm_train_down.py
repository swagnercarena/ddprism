"Perform CLVM analysis of downsampled corrupted MNIST dataset."
import os
...
@jax.jit
def train_step(state, rng, enr_obs, bkg_obs, enr_a_mat, bkg_a_mat, other_vars):
    ...
    def loss_fn(params):
        ...
        enr_loss = state.apply_fn(
            variables, rng_enr, enr_obs, enr_a_mat, method='loss_enr_obs',
            train=True, rngs={'dropout': rng_drop_enr}
        )
        # Background observation loss
        bkg_loss = state.apply_fn(
            variables, rng_bkg, bkg_obs, bkg_a_mat, method='loss_bkg_obs',
    ...
    for _ in tqdm(range(config_clvm.epochs), desc='Training CLVM'):
        for _ in tqdm(range(config_clvm.steps_per_epoch), desc='Steps', leave=False):
            ...
            state, loss = train_step(
                ...
                enr_a_mat[batch_i], bkg_a_mat[batch_i_bkg],
                other_vars
            )
...
def main(_):
    """Run CLVM analysis of downsampled corrupted MNIST digits."""
