...
class BroadcastHEALPixTransformerTest(chex.TestCase):
    """Run tests for the BroadcastHEALPixTransformerTest."""

    @chex.all_variants
    def test_call(self):
        """Test that BroadcastHEALPixTransformerTest returns desired outputs."""
        rng = jax.random.PRNGKey(5)
        emb_features = 4
        n_blocks = 3
        dropout_rate_block = [0.1, 0.1, 0.1]
        ...
        # Initialize HEALPixTransformer
        transformer = embedding_models_healpix.BroadcastHEALPixTransformer(
            emb_features=emb_features,
            n_blocks=n_blocks,
            dropout_rate_block=dropout_rate_block,
            heads=heads,
        ...
        # Test that the model parameters have the correct dimensions (i.e.
        # account for the desire to predict a single frequency).
        self.assertTupleEqual(
            params['params']['Dense_0']['kernel'].shape,
            (patch_size, emb_features)
        )
        output = output.reshape(batch_size, n_pixels, channels)
        self.assertTrue(
            jnp.allclose(output[:, :, 0], output[:, :, 1])
        )
